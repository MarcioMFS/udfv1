...
console.log('📦 Análise detalhada das entregas:\n');
...
console.log('═'.repeat(80));
console.log(`Total de entregas: ${manifest.deliveries.length}`);
console.log(`Bonus Target: ${manifest.bonusTarget}\n`);

// Agrupar por source
const bySource = {};
manifest.deliveries.forEach((d, i) => {
  const sourceName = Object.keys(ARCode).find(key => ARCode[key] === d.source);
  if (!bySource[sourceName]) {
    bySource[sourceName] = { total: 0, satisfied: 0, unsatisfied: 0 };
  }
  bySource[sourceName].total++;
  if (d.satisfaction) {
    bySource[sourceName].satisfied++;
  } else {
    bySource[sourceName].unsatisfied++;
  }

  console.log(`Entrega ${i + 1}:`);
  console.log(`  Source: ${sourceName} (${d.source})`);
  console.log(`  Satisfeita: ${d.satisfaction ? 'SIM ✓' : 'NÃO ✗'}`);
...
Object.entries(bySource).forEach(([source, stats]) => {
  ...
  console.log(`  Satisfeitas: ${stats.satisfied}`);
  console.log(`  Não satisfeitas: ${stats.unsatisfied}`);

  // Calcular bônus para essa localização
  let bonus = 0;
  if (stats.satisfied > manifest.bonusTarget) {
    bonus = stats.satisfied - manifest.bonusTarget;
  } else if (stats.satisfied === manifest.bonusTarget) {
    bonus = 1;
  }
  console.log(`  Bônus desta localização: ${bonus}`);
  console.log('');
});
...
console.log('═'.repeat(80));
const totalSatisfied = manifest.deliveries.filter(d => d.satisfaction).length;
const totalDeliveries = manifest.deliveries.length;
console.log(`Total de entregas satisfeitas: ${totalSatisfied}/${totalDeliveries}`);
console.log(`Satisfação: ${manifest.satisfaction}% (${totalSatisfied}/${totalDeliveries} = ${(totalSatisfied/totalDeliveries*100).toFixed(2)}%)`);
console.log(`Bônus total: ${manifest.bonus}`);
console.log(`Lucro: ${manifest.profit}`);
console.log(`Receita: ${manifest.revenue}`);
console.log(`Custo: ${manifest.cost}`);

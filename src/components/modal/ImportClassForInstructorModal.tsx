...
export function ImportClassForInstructorModal({ isOpen, onClose, onSuccess }: ImportClassForInstructorModalProps) {
  ...
  // Turma que o usuario escolheu ATUALIZAR. null = criar turma nova.
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (previewData.classExists) {
        setSelectedClassId(previewData.matchingClasses[0]?.id ?? null)
  ...
  const performImport = async (classInfo: any, students: any[], events: any[], targetClassId?: string) => {
    if (!selectedInstructorId) {
    ...
    setIsLoading(true)
    try {
      const result = await importClassFromExcel(classInfo, students, events, selectedInstructorId, targetClassId)
      ...
      if (result.success) {
        toast.success(
          `Turma ${targetClassId ? 'atualizada' : 'criada'} com sucesso! ${result.studentsImported} alunos e ${result.eventsImported} eventos cadastrados.`
        )
        onSuccess()
      } else {
  ...
  const handleConfirmUpdate = async () => {
    ...
    try {
      ...
      if (classInfo) {
        if (!selectedClassId) {
          toast.error('Selecione qual turma deseja atualizar')
          return
        }
        await performImport(classInfo, students, eventsWithType, selectedClassId)
      }
    } catch (error) {
  ...
  // Cria uma turma NOVA mesmo havendo turmas parecidas. E o caminho correto
  // quando a planilha e de uma turma de outro semestre com o mesmo nome.
            ...
            <div className="border border-yellow-200 bg-yellow-50 rounded-lg p-4">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-yellow-800 mb-2">
                    Já existe turma parecida
                  </h3>
                  <p className="text-sm text-yellow-700 mb-3">
                    Encontramos {preview.matchingClasses.length}{' '}
                    {preview.matchingClasses.length === 1 ? 'turma' : 'turmas'} com o nome{' '}
                    <strong>{preview.className}</strong>. Se esta planilha é de uma turma NOVA
                    (outro semestre, por exemplo), escolha <strong>Criar turma nova</strong> —
                    atualizar apaga os encontros da turma antiga.
                  </p>

                  <div className="bg-white rounded-md p-3 mb-3">
                    <h4 className="text-sm font-semibold text-gray-700 mb-2">
                      Atualizar qual turma?
                    </h4>
                    <div className="space-y-2">
                      {preview.matchingClasses.map(match => (
                        <label
                          key={match.id}
                        ...
                        >
                          <input
                            ...
                            className="mt-1"
                            checked={selectedClassId === match.id}
                            onChange={() => setSelectedClassId(match.id)}
                          />
                          <span className="text-sm text-gray-700">
                            ...
                            <span className="block text-xs text-gray-500">
                              {match.studentsCount} alunos · {match.eventsCount} eventos
                              {match.createdAt
                                ? ` · criada em ${new Date(match.createdAt).toLocaleDateString('pt-BR')}`
                                : ''}
                            </span>
                          </span>
                        </label>
                  ...
                  <div className="text-xs text-yellow-600 mt-2 bg-yellow-100 rounded p-2 space-y-1">
                    <p className="font-semibold">⚠️ Se você atualizar a turma selecionada:</p>
                    ...
                    <p>• <strong>Turma:</strong> Descrição e instrutor serão atualizados. O código não muda.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-medium disabled:bg-gray-300 disabled:cursor-not-allowed"
                >
                  {isLoading ? 'Criando...' : 'Criar turma nova'}
                </button>
                <button
                ...
                >
                  {isLoading ? 'Atualizando...' : 'Atualizar a selecionada'}
                </button>
              </div>
            </div>
          </div>
